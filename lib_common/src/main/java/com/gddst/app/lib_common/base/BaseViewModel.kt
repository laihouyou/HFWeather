...
//                        Bus.post(USER_LOGIN_STATE_CHANGED, false)
                    ...
                    -1 -> {
                        // 其他api错误
                        BaseApplication.sInstance.showToast(e.message)
                    }
                    else -> {
                        // 其他错误
                        BaseApplication.sInstance.showToast(e.message)
            ...
            is ConnectException -> {
                // 连接失败
                BaseApplication.sInstance.showToast(BaseApplication.sInstance.getString(R.string.network_connection_failed))
            }
            is SocketTimeoutException -> {
                // 请求超时
                BaseApplication.sInstance.showToast(BaseApplication.sInstance.getString(R.string.network_request_timeout))
            }
            is JsonParseException -> {
                // 数据解析错误
                BaseApplication.sInstance.showToast(BaseApplication.sInstance.getString(R.string.api_data_parse_error))
            }
            else -> {
                // 其他错误
                e.message?.let { BaseApplication.sInstance.showToast(it) }
            }
        }
    }
